...
public class TransDetailsApiController : ControllerBase
{
    ...
    [HttpPost("/api/transaksi/detail/kontrak/list")]
    public async Task<IActionResult> KontrakList(long trans)
    {
        var draw = Request.Form["draw"].FirstOrDefault();
        var start = Request.Form["start"].FirstOrDefault();
        var length = Request.Form["length"].FirstOrDefault();
        ...
        var init = repo.Kontraks
            .Where(t => t.TransKegiatanId == trans)
            .Select(x => new
            {
                KontrakId = x.KontrakId,
                NoKontrak = x.NoKontrak,
                Penyedia = x.Penyedia.NamaPenyedia,
                TglMulai = x.TglMulai.ToString(),
                TglBerakhir = x.TglBerakhir.ToString()
            });
        ...
        if (!string.IsNullOrEmpty(searchValue))
        {
            init = init.Where(a => a.NoKontrak.ToLower().Contains(searchValue.ToLower()) ||
                a.Penyedia.ToLower().Contains(searchValue.ToLower())
            );
        }
